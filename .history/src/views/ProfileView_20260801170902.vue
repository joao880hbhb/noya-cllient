<template>
  <div class="min-h-screen bg-gray-50 py-8">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <!-- Loading state -->
      <div v-if="authStore.isLoading" class="flex justify-center items-center h-64">
      ...
      <div v-else-if="authStore.user" class="bg-white shadow rounded-lg">
        ...
        <div class="px-6 py-8 border-b border-gray-200">
          <div class="flex items-center">
            ...
            <div class="ml-6">
              <h1 class="text-3xl font-bold text-gray-900">{{ authStore.userDisplayName }}</h1>
              <p class="text-gray-600">{{ authStore.user.email }}</p>
              <p v-if="authStore.user.bio" class="mt-2 text-gray-700">{{ authStore.user.bio }}</p>
            </div>
          </div>
        </div>
        ...
        <div class="px-6 py-6">
          ...
          <dl class="grid grid-cols-1 gap-x-4 gap-y-6 sm:grid-cols-2">
            <div>
              ...
              <dd class="mt-1 text-sm text-gray-900">{{ authStore.userDisplayName }}</dd>
            </div>
            <div>
            ...
            <div>
              ...
              <dd class="mt-1 text-sm text-gray-900">{{ authStore.user.publicId || 'Not set' }}</dd>
            </div>
            <div>
            ...
            <div v-if="authStore.user.address?.city || authStore.user.address?.country">
              <dt class="text-sm font-medium text-gray-500">Location</dt>
              <dd class="mt-1 text-sm text-gray-900">
                {{ authStore.user.address.city || '' }}{{ authStore.user.address.city && authStore.user.address.country ? ', ' : '' }}{{ authStore.user.address.country || '' }}
              </dd>
            </div>
            <div v-if="authStore.user.socialMedia?.website">
              <dt class="text-sm font-medium text-gray-500">Website</dt>
              <dd class="mt-1 text-sm text-gray-900">
                <a :href="authStore.user.socialMedia.website" target="_blank" class="text-indigo-600 hover:text-indigo-500">
      ...
      <!-- Error state -->
      <div v-else-if="authStore.error" class="bg-red-50 border border-red-200 rounded-lg p-6">
