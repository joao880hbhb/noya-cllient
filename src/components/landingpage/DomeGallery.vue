        ...
        >
          ...
          >
            <div
              ...
              role="button"
              tabindex="0"
              :aria-label="item.alt || item.title || 'Open image'"
              @click="onTileClick"
...
/**
 * ImageItem sekarang mendukung metadata blog:
 * - title  -> judul postingan, ditampilkan sebagai caption saat gambar diperbesar
 * - author -> nama penulis, ditampilkan di caption
 * - link   -> url/slug postingan. Jika terisi, overlay yang diperbesar bisa diklik
 *             untuk memicu event `select` (parent yang menentukan navigasinya)
 */
interface ImageItem {
  src: string;
  alt?: string;
...
interface DomeGalleryProps {
  ...
  grayscale?: boolean;
  /** Tampilkan caption judul/penulis di gambar yang diperbesar (default: true) */
  showCaption?: boolean;
  /** Teks tombol navigasi ke postingan (kirim hasil t('login.readMore') dari parent untuk i18n) */
...
const props = withDefaults(defineProps<DomeGalleryProps>(), {
  ...
  segments: 35,
  dragDampening: 2,
  openedImageWidth: 'min(86vw, 320px)',
  openedImageHeight: 'min(65vh, 420px)',
...
/**
 * Emit `select` ketika gambar yang sedang diperbesar (yang punya `link`) diklik.
 * Parent bebas menentukan cara navigasinya, misalnya:
 *   <DomeGallery :images="domeImages" @select="(item) => router.push(item.link)" />
 */
const emit = defineEmits<{
  (e: 'select', item: { src: string; title?: string; author?: string; link?: string }): void;
}>();
...
function buildItems(pool: (string | ImageItem)[], seg: number): TileItem[] {
  ...
  const normalizedImages: ImageItem[] = pool.map(img =>
    typeof img === 'string'
      ? { src: img, alt: '' }
      : { src: img.src || '', alt: img.alt || '', title: img.title, author: img.author, link: img.link }
  );
...
const openItemFromElement = (el: HTMLElement) => {
  ...
  const rawAuthor = parent.dataset['author'] ?? '';
  ...
  // Caption (judul + penulis) + tombol eksplisit untuk navigasi ke postingan
  if (props.showCaption && (rawTitle || rawAuthor || rawLink)) {
    ...
    if (rawTitle || rawAuthor) {
      ...
      if (rawTitle) {
        ...
        titleEl.style.cssText = 'color:#fff;font-weight:700;font-size:15px;line-height:1.3;margin-bottom:4px;';
        textWrap.appendChild(titleEl);
      }
      if (rawAuthor) {
        const authorEl = document.createElement('div');
        authorEl.textContent = rawAuthor;
        authorEl.style.cssText = 'color:rgba(255,255,255,0.85);font-size:12px;';
    ...
    if (rawLink) {
      ...
      detailBtn.textContent = props.readMoreLabel;
      ...
      detailBtn.addEventListener('click', ev => {
        ev.stopPropagation();
        emit('select', { src: rawSrc, title: rawTitle || undefined, author: rawAuthor || undefined, link: rawLink });
      });
      caption.appendChild(detailBtn);
    }
